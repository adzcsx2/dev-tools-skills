...
COMMON_SKILLS="init init-root install-project-hooks study push execute-loop update-remote-plugins code-note to-public-cloudflare work-report local-worktree update-docs codex-sync-push codex-sync-pull"
...
category_desc() {
  case "$1" in
    common)  echo "Common tools (dt:init, dt:init-root, dt:install-project-hooks, dt:study, dt:push, dt:execute-loop, dt:update-remote-plugins, dt:code-note, dt:to-public-cloudflare, dt:work-report, dt:local-worktree, dt:update-docs, dt:codex-sync-push, dt:codex-sync-pull)" ;;
