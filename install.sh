...
VSCODE_PROMPT_MARKER="<!-- dev-tools-skills-generated -->"
...
# commands such as do-all are owned through VSCODE_PROMPT_MARKER instead, so
...
install_vscode_prompt() {
  ...
  for dest_file in "$VSCODE_PROMPTS_DIR"/*.prompt.md; do
    ...
    if ! grep -Fq "$VSCODE_PROMPT_MARKER" "$dest_file"; then
