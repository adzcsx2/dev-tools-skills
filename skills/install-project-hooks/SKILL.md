...
| --- | --- | --- | --- | --- |
| `final-rule-audit` | `Stop` | `.claude/hooks/final-rule-audit.sh` | `.codex/hooks/final-rule-audit.sh` | fail-open by default |
...
- 检查 `hooks.Stop` 是否已包含 `.claude/hooks/final-rule-audit.sh`
...
- 读取现有配置
- 检查 `hooks.Stop` 是否已包含 `.codex/hooks/final-rule-audit.sh`
- 若已包含，跳过；若未包含，增量合并，不覆盖其他 hook 条目
- 若文件损坏且无法解析，先备份再重建
...
3. If `--dry-run`：输出目标文件、将注册的 hook、合并策略和验证项后停止
4. Generate Claude hook files when selected
5. Generate Codex hook files when selected
6. Merge existing JSON config incrementally
7. `chmod +x` generated hook scripts
8. Verify files and hook registration
...
- Claude selected 时，`.claude/settings.json` 存在且 `hooks.Stop` 注册 `final-rule-audit.sh`
- Codex selected 时，`.codex/hooks.json` 存在且 `hooks.Stop` 注册 `final-rule-audit.sh`
