...
## 硬约束：init-root 根目录只本地提交不 push

如果当前目录存在 `.ai/init-root.yml`，且其中包含 `root_git_policy: commit_only_no_push`，当前仓库视为 `dt:init-root` 初始化出的多仓库产品根目录：

- root 仓库只保存 AI 规则、文档、`.gitignore`、`.ai/` 等协调状态
- root 仓库允许执行 Step 4 的本地提交，但**禁止执行 Step 1 pull、Step 4.5 squash、Step 5 push 和 tag push**
- root 仓库没有 remote 是正常状态，不应因缺少 `origin` 退出
- 子项目代码必须进入各自子目录后分别执行 `dt:push`，由子项目自己的 remote 推送
...
- 如果命中 `.ai/init-root.yml` 的 root-only policy，跳过 remote/upstream 依赖检查，只处理本地 commit-only 流程

### Step 0.5: init-root Root Commit-Only Fast Path

如果当前目录存在 `.ai/init-root.yml` 且包含 `root_git_policy: commit_only_no_push`：

1. 确认当前目录是 git 仓库；如果不是，提示先执行 `dt:init-root`
2. 读取 `.gitignore`，确认存在 `# dt:init-root child repositories` block
3. 检查 `git status --porcelain`，如果出现直接子级 git 项目内容，停止并提示先修正 `.gitignore`
4. 如果 `--preview`，只展示将提交的 root 文件和 commit message，不执行写入
5. 如果工作区无变更，报告 root 没有需要本地提交的内容，并退出
6. 执行 `git add .`，但不得 stage 被 `.gitignore` 忽略的子项目目录
7. 创建 1 个本地 commit，commit message 使用中文，例如 `docs: 更新多仓库根目录初始化状态`
8. 完成后输出：root 已本地 commit，未 push；如需推送子项目，请进入对应子项目目录执行 `dt:push`

命中本 fast path 后，**不得继续执行 Step 1 至 Step 5**。
