...
- 逻辑分组策略包含 P0、P1、P1.5、P2、P3、P4 六个优先级：字符串替换、符号重命名、TDD / 修复需求功能包、目录聚类、同主题新增、独立变更
- TDD / 修复需求场景下（P1.5 优先级），同一需求的实现、对应测试用例、需求总结文档必须合并为 1 个逻辑 commit；只有与当前需求无关的测试基础设施、通用测试工具或独立测试维护才单独作为 `test` commit；通过显式关联校验（模块路径/符号引用/特征词匹配）判定，不做简单目录归并
...
| `references/commit-grouping.md`     | 工作区变更逻辑分组算法、TDD / 修复需求功能包、主题纯度校验、commit message 规则               |
