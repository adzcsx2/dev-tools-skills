---
name: dt:init-root
description: "Initialize a multi-repository project root by running dt:init first, then dt:update-docs, then configuring root commit-only/no-push behavior, child-repository ignores, root-level dt:push boundaries, and project-level Claude/Codex hooks. Use when a parent folder represents one product made from frontend/backend/other child repositories."
...
`dt:init-root` 是多仓库产品根目录初始化编排入口。它不复制 `dt:init` 的内部流程；执行时必须先完整执行 `dt:init`，包含 Claude/Codex project hook 初始化，再完整执行 `dt:update-docs`，最后只追加根目录多仓库差异规则。
...
- Preserve its CodeGraph, docs taxonomy, `.ai/skills/`, `CLAUDE.md`, `AGENT.md`, Copilot config, Claude/Codex project hooks, final rule audit hook, review, and `.gitignore` behavior
...
- Claude/Codex project hook files generated by `dt:init` still exist unless `--dry-run`
