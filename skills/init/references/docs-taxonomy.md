...
位置约定：`docs/plan/<中文任务名>/`，可保留稳定日期后缀（例如 `-2026-07-09`）。

最低结构要求：

- `任务说明.md`：任务背景、文档清单、建议阅读顺序、关联源码、任务状态
- `00-执行文档.md`：进度指针、断点续做协议、Phase checklist；详细执行日志文件统一写入项目根 `logs/`

其他约定：

- 任务聚合子目录内的文档不重复落到 `docs/design/`、`docs/guide/` 等顶级分类
- 任务聚合文档不直接修改项目单一事实源
- `docs/文档索引.md` 的计划文档章节必须列出活跃任务子目录
...
- 审计 / 性能 / 评估 / 复盘类报告默认使用 `docs/reports/<中文报告主题>/`
- `CHANGELOG.md` 这类持续更新日志可保留在 `docs/reports/` 根下
- 运行、调试、执行和审计日志文件统一写入项目根 `logs/`；不要在 `/docs` 或仓库根目录散落日志文件
