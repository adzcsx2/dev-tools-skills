...
最终结果必须同时服务通用 AI 规则、Claude / Copilot 等规则文件，以及项目里已经配置好的 tool mirrors，并满足以下目标：
...
7. 项目级 skill 只同步到“已配置”的工具；未配置工具不创建镜像
...
6. 已配置 tool mirrors 的检测结果与必要派生层预览或落盘结果
7. `/docs` 文档根目录及必要的分类目录骨架
...
标准 init 完成后，必须在目标项目内建立最小 project-skills 工作骨架，作为项目级 AI skill 的 canonical source；同时要检测项目里哪些 tool mirrors 已经配置，并只为这些工具建立或刷新派生层。

### 3.6.1 Configured Tool Detection

先检测项目里哪些 AI 工具已经有 project-skills 接收面。判断依据必须来自真实文件、目录或用户明确要求，不得凭空假设。

判断顺序必须是：

1. 若 `.ai/README.md` 已存在，先读取其中记录的 configured tool mirrors；这份记录是项目级镜像配置的一手说明
2. 再看仓库里已有的工具镜像或工具约定目录
3. 再看用户是否在当前需求里明确要求启用某个工具 mirror
4. 兼容旧项目：如果已存在 legacy `tool_exports: [claude|codex]` 记录，也视为该工具原本已启用
5. 最终把确认后的 configured tools 写回 `.ai/README.md`，供后续 `dt:project-skills` 优先复用

判定约束：

- 如果 `.ai/README.md` 已明确列出 configured tools，步骤 2-4 只能作为补充启用信号，不能因为单次导出目录或偶发残留目录就静默扩张工具集合
- `.ai/README.md` 中未列出的工具，默认视为当前项目未启用；只有用户在本次需求里明确要求启用，或检测到 legacy `tool_exports` 明确声明，才允许加入 configured tools
...
- `CLAUDE.md`、`AGENT.md`、`AGENTS.md`、`.github/copilot-instructions.md` 这类规则文件本身不自动等价为“已启用 tool mirror”
- 如果项目里没有任何可确认的 tool mirror，就不要创建 `.claude/skills/`、Codex 导出目录或其他工具镜像
- 检测结果必须写进 `.ai/README.md`，供后续 `dt:project-skills` 优先读取
- 单次 `export <tool>` 生成的 view 本身不自动等价为“长期已配置 mirror”；只有 init 记录或用户明确启用 mirror 后，后续才持续自动同步

### 3.6.2 Canonical Location
...
- 若 `.ai/README.md` 不存在，必须创建，并写明当前 configured tool mirrors
...
- 只有检测到 `claude` 时，才创建或刷新 `.claude/skills/`
- 只有检测到 `codex` 时，才创建或刷新 `.ai/exports/codex/`（或项目已有明确 Codex 位置）
...
- `tool_exports: follow_configured_tools` 表示默认跟随项目里已配置的工具集合
- 若后续某个 skill 需要只同步部分工具，才把 `tool_exports` 改成显式列表；实际同步目标仍必须与“已配置工具集合”求交集
...
- 写清默认跟随已配置工具自动同步；未配置工具不创建镜像
- 写清任何 tool export 或 mirror 都不能反向成为事实源
...
- Sync mirrors only for configured tools
- Do not edit tool export layers directly
- Refresh configured mirrors from `.ai/skills/`
...
- If no tool mirror is configured, update canonical only
```

### 3.6.3 Bootstrap Rules

默认建立 canonical-first 的项目级接入：`.ai/skills/` 是唯一事实源；工具镜像是否存在，取决于项目里是否已经配置对应 mirror。

这意味着：

- init 默认创建 `.ai/skills/` canonical source
- init 默认把 configured tool mirrors 记录写进 `.ai/README.md`
- init 必须把 project skill 规则写进生成的 `CLAUDE.md` 和 `AGENT.md`
- init 不假设 Claude 或 Codex 镜像一定存在
- 如果只检测到 `claude`，就只维护 `.claude/skills/`
- 如果同时检测到 `claude` 和 `codex`，就同时维护这两个派生层
- 如果没有检测到任何已配置工具，就只创建 canonical source，不创建任何工具镜像
- 如果检测到 legacy `tool_exports: [claude]`、`[codex]`，即使当前 mirror 目录缺失，也允许在本次 init 重新补建对应 mirror
- 如果本次带 `--dry-run`，`.ai/` 骨架和所有工具镜像都只允许输出预览，不得实际创建目录或文件

### 3.6.4 Canonical-Only Rule

生成的项目级规则文件必须显式写入以下约束：

- 修改、沉淀、合并项目级 skill 时，只允许修改 `.ai/skills/` 下的 canonical 文件
- `.claude/skills/`、`.ai/exports/codex/` 或其他工具侧文件都是导出层，不是事实源
- 项目级 skill 更新后，只同步到当前项目里已配置的工具；未配置工具不创建镜像
...
8. 真实目录结构、默认构建与测试方式
9. 项目级 skill 的唯一事实源是 `.ai/skills/`，修改 skill 时只改 canonical source，不手改导出层；如有已配置工具，则自动同步到这些工具
...
5. 项目级 skill 规则：`.ai/skills/` 是唯一事实源，只改 canonical source；工具侧 mirror 只对已配置工具自动同步
...
- 如果项目已建立 `.ai/skills/`，必须补一句：项目级 skill 只改 `.ai/skills/` canonical source，不手改导出层；已配置工具才接收同步
...
- 我想改哪里该看哪里
- 文档应该放在哪个 `/docs` 分类目录
- 项目级 skill 在 `.ai/skills/`；如项目已配置 Claude / Codex 等 tool mirror，则后续更新会自动同步到这些工具，否则只维护 canonical source
