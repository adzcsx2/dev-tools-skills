...
当某一具体工作项（bug 修复、重构、特性落地、审计后修复）会产出**多份相互关联的文档**（例如 设计 + 规范 + 计划 + 测试计划 + 测试用例）时，必须采用"任务聚合子目录"模式，而不是把这些文档分散到顶级分类下。

**位置约定**：

- 任务聚合子目录位于 `/docs/plan/<task-slug>/`
- `<task-slug>` 使用英文 kebab-case，语义稳定，例如 `ble-multi-device-fix`、`home-card-migration`、`auth-rewrite-2026q2`
- 除非需要消歧，否则 slug 不带日期

**结构要求**：

- 子目录下必须有 `README.md` 作为索引入口，包含：任务背景、文档清单、建议阅读顺序、关联源码、关联上游文档、任务状态
- 子目录下**必须**有 `00-执行文档.md` 作为 AI 入口，内容包含：
  - 当前进度指针（`<!-- progress-pointer:start -->` / `<!-- progress-pointer:end -->` 包裹的 YAML 块，含 `current_phase` / `current_phase_status` / `last_updated` / `last_actor` / `last_commit` / `next_action` / `blockers`）
  - 断点续做协议（6 步：读指针 → 对齐状态 → 从未完成项继续 → 完成后更新指针 → 遇阻塞停止 → 切 Phase 门槛）
  - 子代理规划表（Phase × 职责 × 推荐 subagent，按栈差异化）
  - Phase 详细 checklist（原子化、可验证）
  - 执行日志（倒序追加事件记录）
  - 执行指令模板（供用户/AI 构造 prompt）
- 子目录内容文档用编号前缀标明阅读顺序，`00-` 保留给执行文档；`01-架构设计.md`、`02-开发规范.md`、`03-修复路线图.md`、`04-测试计划.md`、`05-测试用例清单.md`
- 子目录内部可以自由组织小节，但不得再嵌套"任务聚合子目录"
- AI 进入任务目录必须先读 `00-执行文档.md` 的进度指针，再决定从哪里继续，不得重头开始

**与顶级分类的关系**：

- 任务聚合子目录内的文档**不重复落到** `docs/design/`、`docs/guide/`、`docs/checklist/` 等顶级分类
- 跨任务共享的长期文档（通用开发规范、架构总览、模块说明、SDK 参考）**仍然**走顶级分类，不进任务子目录
- 任务聚合文档**不直接修改项目单一事实源**（如 `.cursor/rules/*.mdc`、顶级 `docs/guide/*使用规范.md`）；稳定结论在任务完成后由单独的反哺任务合入事实源

**触发条件**：

- 一个工作项会产出 ≥3 份关联文档 → 使用任务聚合子目录
- 一个工作项只产出 1~2 份独立文档 → 按传统分类归档
- 不要为单文档强行创建任务子目录

**推荐生成入口**：

- 若项目已安装 `dev-tools-skills` 插件，可调用 `/dt:plan-doc <task-slug>` 自动生成完整任务聚合文档集（含 `00-执行文档.md` 的进度指针和子代理规划）
- 若需要测试文档，使用 `/dt:plan-doc <task-slug> test`，或在 prompt 中包含"测试 / 回归 / 自测 / QA / 验证 / 用例"关键词触发
- 未安装 `dt:plan-doc` 时，按上述结构要求手动创建所有必选文件，不得省略 `00-执行文档.md`

**docs 索引维护**：

- `docs/README.md` 的"计划文档"章节必须列出当前活跃的任务聚合子目录及其 README 链接
- 任务完成归档后，子目录保留作为历史证据，索引可转入"已归档任务"小节
...
- **任务聚合子目录规则（3.5.5）必须在 CLAUDE.md、AGENT.md 及 Copilot 项目级配置中以精简形式写入**：当一个工作项会产出多份关联文档时，统一放到 `docs/plan/<task-slug>/` 子目录并配 `README.md` 索引；不分散到顶级分类；不修改单一事实源
...
- **必须写入任务聚合子目录规则**（见 Phase 3.5.5）：当一个工作项会产出 ≥3 份关联文档时，统一聚合到 `docs/plan/<task-slug>/` 子目录；子目录内必须有 `README.md` 索引；任务聚合文档不得直接修改单一事实源
...
4. **关键路径索引**
   ...
   - 公共组件/工具类位置
   - 文档目录结构
   - 任务聚合子目录约定：多文档工作项统一放 `docs/plan/<task-slug>/`，含 `README.md` 索引，不分散到顶级分类
...
- 必须包含精简版文档归档规则：文档放 `/docs`、先查现有分类、避免创建同义目录；多文档工作项聚合到 `docs/plan/<task-slug>/` 并配 README 索引
...
- 跨模块或高风险改动不先计划就直接执行
- 工作项会产出多份关联文档时，仍把它们分散到 `docs/design/`、`docs/guide/`、`docs/checklist/` 等顶级分类而不聚合到 `docs/plan/<task-slug>/`
- 任务聚合子目录缺少 `README.md` 索引
- 任务聚合子目录缺少 `00-执行文档.md` 或其进度指针 YAML 块
- 进度指针未用 `<!-- progress-pointer:start -->` / `<!-- progress-pointer:end -->` 锚点包裹
- AI 进入任务目录不先读进度指针就开工，重头执行已完成的 Phase
- 把进度指针更新委托给 subagent（必须由 main-agent 自己维护）
- 任务聚合子目录内的文档直接修改单一事实源（`.cursor/rules/*.mdc`、顶级使用规范 md）
