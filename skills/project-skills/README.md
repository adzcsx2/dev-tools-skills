...
统一的项目级 AI skill 生命周期入口。以 `.ai/skills/` 为唯一事实源，并默认把 Claude 项目级镜像同步到 `.claude/skills/`，用于列出项目 skill、审计重复和重叠、在确认后同步更新、把成功实现沉淀成 skill，以及按需导出 Copilot / Codex 适配层。
...
- 不直接手改 `.claude/skills/`、Copilot、Codex 等导出层
