...
$VSCodePromptMarker = "<!-- dev-tools-skills-generated -->"
...
$CommonSkills = @("do-all", "init", "init-root", "install-project-hooks", "study", "push", "execute-loop", "update-remote-plugins", "code-note", "to-public-cloudflare", "work-report", "local-worktree", "update-docs", "codex-sync-push", "codex-sync-pull")
$AndroidSkills = @("gradle-build-performance", "android-i18n", "android-fold-adapter")
$FlutterSkills = @()
...
function Get-CategoryDesc($cat) {
    switch ($cat) {
        "common"  { "Common tools (do-all, dt:init, dt:init-root, dt:install-project-hooks, dt:study, dt:push, dt:execute-loop, dt:update-remote-plugins, dt:code-note, dt:to-public-cloudflare, dt:work-report, dt:local-worktree, dt:update-docs, dt:codex-sync-push, dt:codex-sync-pull)" }
        "android" { "Android tools (adt:gradle-build-performance, adt:android-i18n, adt:android-fold-adapter)" }
...
# Derive only explicit namespace prefixes (e.g. dt, adt, fdt). Unprefixed
# commands such as do-all are owned through $VSCodePromptMarker instead, so
# they never broaden legacy prefix cleanup to unrelated do-*.prompt.md files.
function Get-InstallerOwnedPrefixes {
    ...
    foreach ($skillDir in $skillDirs) {
        ...
        if (-not (Test-Path $mdFile)) { continue }
        $cmdName = Get-SkillName $mdFile
        if ($cmdName -and $cmdName.Contains(':')) {
            $prefixes[$cmdName.Split(':', 2)[0]] = $true
        }
    }
    return $prefixes
}
...
function Install-VSCodePrompt {
    ...
    foreach ($skillDir in $skillDirs) {
        ...
        $skillContent = Get-Content $mdFile -Raw
        [System.IO.File]::WriteAllText(
            $targetPath,
            "$VSCodePromptMarker`n$skillContent",
    ...
    # Clean up stale prompts generated by this installer. Marker-based ownership
    # handles unprefixed commands; explicit prefixes preserve compatibility with
    # older generated files that did not contain the marker.
    ...
    foreach ($dpf in $destPromptFiles) {
        ...
        $isMarked = Select-String -LiteralPath $dpf.FullName -SimpleMatch $VSCodePromptMarker -Quiet
        if (-not $isMarked -and -not $ownedPrefixes.ContainsKey($prefix)) { continue }
